...
/*
CODIGO COM CAPTURA EM PASSANT (AINDA NAO FINALIZADA)
...
hasWhiteLeftRookMoved = false;
hasWhiteRightRookMoved = false;
let peaoParaCapturaEnPassant = null;
...
function possiveis_movimentos() {
    ...
    if (peca[x][y]['peca'] === 'peao') {
        if (peca[x][y]['cor'] === "branco") {
            ...
            if (x === 7 && !peca[x - 2][y]['peca'] && !peca[x - 1][y]['peca']) {
                possivel(x - 2, y);
                // Registrar o peão que pode ser capturado en passant
                peaoEnPassant = { x: x - 1, y: y }; // Posição do peão que se moveu duas casas
            }
        } else if (peca[x][y]['cor'] === "preto") {
            // Movimento de 1 casa para frente
            if (!peca[x + 1][y]['peca']) {
...
function seleciona(x, y) {
    if ((movimenta['selecionada']['x'] == 0 || peca[x][y]['cor'] == movimenta['selecionada']['cor']) && peca[x][y]['cor'] == vez) {
        ...
        if (peca[x][y]['cor'] != movimenta['selecionada']['cor']) {
            ...
            // Captura "en passant"
            if (peca[x][y]['peca'] === 'peao' && peca[x][y]['cor'] !== movimenta['selecionada']['cor'] && peaoEnPassant) {
                if ((movimenta['selecionada']['cor'] === 'branco' && x === peaoEnPassant.x - 1 && (y === peaoEnPassant.y - 1 || y === peaoEnPassant.y + 1)) ||
                    (movimenta['selecionada']['cor'] === 'preto' && x === peaoEnPassant.x + 1 && (y === peaoEnPassant.y - 1 || y === peaoEnPassant.y + 1))) {

                    // Realiza a captura "en passant"
                    document.getElementById("t" + peaoEnPassant.x + peaoEnPassant.y).innerHTML = ""; // Remove o peão que foi capturado
                    peca[peaoEnPassant.x][peaoEnPassant.y] = { peca: false, cor: false }; // Remove a peça da posição
                }
            }
